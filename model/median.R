...
get_result_median <- function(df, i) {
  ...
  # Predict
  y_train <- df_train$y
  y_train_pred <- rep(median(df_train$y, na.rm = TRUE), length(y_train))
  y_test <- df_test$y
  y_test_pred <- rep(median(df_train$y, na.rm = TRUE), length(y_test))
...
df <- df %>%
  ungroup() %>%
  mutate(`43` = future_map(data, get_result_median, 1),
         `40` = future_map(data, get_result_median, 2),
         `37` = future_map(data, get_result_median, 3),
         `34` = future_map(data, get_result_median, 4))
res <- df %>%
  select(-data) %>%
  gather(cv, res, -site_code, -product_code)

write_rds(res, "data/temp/median.rds")
...
res <- read_rds("data/temp/median.rds")
res <- res %>%
