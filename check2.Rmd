...
Let's visualize for each cluster, show top 50 based on different features

```{r}
# Return top 
viz_cluster <- function(cluster_final, cluster_idx = 8) {
  p <- list()
...
```{r}
summary_mase %>% 
  filter(cnt_v3_no_zero <= 0.5) %>% 
  arrange(desc(linearity)) %>% 
  head(10) %>% 
  viz_cluster()
```
