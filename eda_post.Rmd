...
visualize_specific_prediction <- function(res_df,
  ...
  p <- train %>% 
    ...
    ggplot(aes(ds, stock_distributed, color = cat)) +
    geom_line() +
    labs(title = paste(rank, site, product, "cluster", cl, sep = "-"))
  p
}
...
res_lgb_final <- read_prediction("submission/data/ensemble1.csv", "LGB-ia-2")
...
avg_best_selection <- summary_res %>% 
  ...
  # filter(method %in% c("LGB-ia", "LGB-i", "LGB", "Theta", "ETS", "ARIMA")) %>%
  filter(!(method %in% c("NN"))) %>% 
  filter(block <= 40) %>%
  # filter(block == 42) %>%
...
It is getting worsen, let's find a way to select the best approach :)

```{r avg-best-selection}
# Should be significant on three CV :)
rank_avg_best_selection <- summary_res %>% 
  filter(block <= 40) %>% 
  filter(block %in% c(34, 37, 40)) %>% 
  mutate(default_model = case_when(method == "LGB-ia" ~ 1,
                                   method == "Naive" ~ 4,
                                   method == "SNaive" ~ 5,
                                   method == "ETS" ~ 2,
  ...
  mutate(r = row_number()) %>% 
  ungroup() %>% 
  arrange(site_code, product_code, method, block) %>% 
  group_by(site_code, product_code, method) %>% 
  summarise(mase_mean = mean(mase),
            mae_mean = mean(mae),
            rank_avg = mean(r))

# View consistent model
# rank_avg_best_selection %>% 
#   filter(rank_avg < 2) %>% 
#   arrange(desc(mase_mean))

# Select best method
avg_best_selection <- summary_res %>% 
  filter(method == "LGB-ia") %>% 
  filter(block == 43) %>%
  anti_join(rank_avg_best_selection %>% 
              filter(rank_avg <= 1) %>% 
              select(site_code, product_code)) %>% 
  bind_rows(
    summary_res %>% 
      filter(block == 43) %>%
      inner_join(rank_avg_best_selection %>% 
                   filter(rank_avg <= 1) %>% 
                   select(site_code, product_code, method))
  )
avg_best_selection %>% 
  count(method)
avg_best_selection %>% 
...
Again, the improvement is not really significant
...
```{r cluster-mase}
summary_cluster <- summary_res_mase_spread %>% 
...
```{r}
visualize_specific_prediction(res_lgb_final,
                              "C1077",
                              "AS27000")
visualize_specific_prediction(res %>% filter(method == "ARIMA"),
                              "C1077",
                              "AS27000")
```
...
```{r}
# High difference with median
visualize_specific_prediction(res_lgb_final,
                              "C1090",
                              "AS27000")
# High difference normalized
visualize_specific_prediction(res_lgb_final,
                              "C1051",
                              "AS27133")
visualize_specific_prediction(res_lgb_final,
...
visualize_specific_prediction(res_lgb_final,
                              "C1011",
                              "AS27133")
visualize_specific_prediction(res %>% filter(method == "Regression"),
                              "C1011",
                              "AS27133")
```

Find prediction below median value, do median benchmark
...
res_lgb_final %>% 
  ...
  mutate(below_median = preds < val_median) %>% 
  group_by(below_median, block) %>% 
  summarise(n = n(),
            mae = mean(ae))
```
...
res_lgb_final %>% 
  left_join(median_summary) %>% 
  mutate(below_median = preds < val_median*0.5,
         preds = ifelse(preds < val_median*0.5, preds + 0.5*val_median, preds),
         ae = abs(stock_distributed - preds)) %>% 
  group_by(below_median, block) %>% 
  summarise(n = n(),
            mae = mean(ae))
...
res_lgb_final %>% 
  left_join(median_summary) %>% 
  mutate(below_median = preds < val_median,
         preds = ifelse(preds < val_median, preds + (val_median), preds),
         ae = abs(stock_distributed - preds)) %>% 
  group_by(below_median, block) %>% 
  summarise(n = n(),
            mae = mean(ae))
